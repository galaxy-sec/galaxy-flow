...
impl GitTools {
    pub fn new(force: bool) -> ExecResult<Self> {
        let mut ctx = WithContext::want("init git tools");
        let ee = EnvExpress::from_env();
        ctx.with("GX_ROOT", GX_ROOT);
        ctx.with("VENDOR_ROOT", VENDOR_ROOT);
        let rg_root = ee.eval(GX_ROOT).with(&ctx)?;
        let vendor_root = ee.eval(VENDOR_ROOT).with(&ctx)?;
        Ok(GitTools {
            force,
            gx_root: rg_root,
            vendor_root,
    ...
    pub fn pull_mod(&self, url: &str, repo: &str, tag: &str, opt: &ShellOption) -> ExecResult<()> {
        ...
        let cmd = format!(
            "{}/{} {} {}-{} {} {} {}",
            self.gx_root, SH_NAME, url, repo, tag, tag, update, self.vendor_root
        );
    ...
    pub fn pull_init(&self, url: &str, repo: &str, tag: &str, opt: &ShellOption) -> NER {
        ...
        let cmd = format!(
            "{}/{} {} {} {} {} {}",
            self.gx_root, SH_NAME, url, repo, tag, update, self.gx_root
        );
    ...
    fn build_remote_git(&self) -> NER {
        let sh_path = format!("{}/{}", self.gx_root, SH_NAME);
    ...
    pub fn check_run(&self) -> ExecResult<()> {
        ...
        let cmd = format!("{}/{}", self.gx_root, "git_check.sh");
    ...
    fn build_check_shell(&self) -> NER {
        ...
        build_shell(self.gx_root().as_str(), sh_name, shell, sh_path.as_str())
    }
}
