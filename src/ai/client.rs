...
impl AiClient {
    ...
    fn build_enhanced_system_prompt(&self, base_prompt: &str, role: AiRole) -> String {
        ...
        match role {
            AiRole::Developer => {
                enhanced_prompt.push_str("\n【开发者规则】\n");
                enhanced_prompt.push_str("• 使用场景：代码生成、代码审查、架构设计、技术问题解决、文档生成\n");
                enhanced_prompt.push_str("• 约束条件：最大代码长度2000行，必须包含错误处理，需要代码审查\n");
                enhanced_prompt.push_str("• 输出要求：格式良好的代码，包含注释和测试，优雅的错误处理\n");
                enhanced_prompt.push_str("• 最佳实践：验证生成的代码，遵循编码规范，避免敏感代码\n");
                enhanced_prompt.push_str("• 禁止行为：不生成包含硬编码密钥的代码，不生成不安全的代码模式\n");
            },
            AiRole::Operations => {
                enhanced_prompt.push_str("\n【运维人员规则】\n");
