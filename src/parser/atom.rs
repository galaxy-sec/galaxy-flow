...
use winnow::{
    ascii::multispace0,
    combinator::{cut_err, delimited},
    error::{ContextError, StrContext},
    token::{literal, take_while},
    ModalResult, Parser,
};

pub fn take_host(input: &mut &str) -> ModalResult<String> {
    let key =
        take_while(1.., ('0'..='9', 'A'..='Z', 'a'..='z', ['.', '_', '-'])).parse_next(input)?;
    Ok(key.to_string())
}
pub fn take_filename(input: &mut &str) -> ModalResult<String> {
    let key =
        take_while(1.., ('0'..='9', 'A'..='Z', 'a'..='z', ['.', '_', '-'])).parse_next(input)?;
    Ok(key.to_string())
}
pub fn take_filename_body(input: &mut &str) -> ModalResult<String> {
    let key = take_while(1.., ('0'..='9', 'A'..='Z', 'a'..='z', ['_', '-'])).parse_next(input)?;
    Ok(key.to_string())
}

// 提取受限 KEY (只包字符)
pub fn take_limit_key(input: &mut &str) -> ModalResult<String> {
    let key = take_while(1.., ('0'..='9', 'A'..='Z', 'a'..='z')).parse_next(input)?;
    Ok(key.to_string())
}

//take a.b => (a,b)
pub fn take_dot_pair(input: &mut &str) -> ModalResult<(String, String)> {
    // 解析第一个部分
...
//take ${var_name}
pub fn take_var_ref_name(input: &mut &str) -> ModalResult<String> {
    // 使用 delimited 解析 ${var_name}
    delimited(
        "${",                   // 开头标记
        cut_err(take_var_name), // 解析变量名
        "}",                    // 结尾标记
    )
