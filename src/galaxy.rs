...
impl Galaxy {
    ...
    pub fn env_init() -> RunResult<()> {
        ...
        if !galaxy_dir.exists() {
            std::fs::create_dir_all(&galaxy_dir).owe(UvsReason::resource_error().into())?;
        }
    ...
    pub fn project_init() -> RunResult<()> {
        ...
        std::fs::create_dir_all(gal_dir).owe(UvsReason::resource_error().into())?;

        // Create basic work.gxl from template
        let work_gxl = gal_dir.join("work.gxl");
        std::fs::write(&work_gxl, WORK_TEMPLATE).owe(UvsReason::resource_error().into())?;
        eprintln!("created: {}", work_gxl.display());

        // Create basic adm.gxl from template
        let adm_gxl = gal_dir.join("adm.gxl");
        std::fs::write(&adm_gxl, ADM_TEMPLATE).owe(UvsReason::resource_error().into())?;
